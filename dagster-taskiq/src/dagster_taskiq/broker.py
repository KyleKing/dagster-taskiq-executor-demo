...
class LocalStackSqsBroker:
    ...
    async def startup(self) -> None:
        """Start the broker."""
        self.logger.info("Starting LocalStack SQS broker for queue: %s", self.queue_name)
        ...
        try:
            ...
            # Get queue URL
            response = await self.sqs_client.get_queue_url(QueueName=self.queue_name)  # type: ignore[attr-defined]
    ...
    async def send_message(self, message: Any) -> None:
        ...
        try:
            ...
            self.logger.debug("Message sent to queue: %s", self.queue_name)
        except Exception as e:
            self.logger.exception("Failed to send message to queue: %s", e)
            raise
