"""Command-line interface for dagster-taskiq.

This module provides CLI commands for running Taskiq workers and managing
...
from dagster._config import post_process_config, validate_config  # noqa: PLC2701
from dagster._core.errors import DagsterInvalidConfigError  # noqa: PLC2701
from dagster._core.instance import DagsterInstance  # noqa: PLC2701
from dagster._utils import mkdir_p  # noqa: PLC2701
...
def create_worker_parser(subparsers: argparse._SubParsersAction[Any]) -> argparse.ArgumentParser:  # pyright: ignore[reportPrivateUsage]
    ...
    Args:
        subparsers: The subparsers action to add the worker parser to

    Returns:
        The created worker parser
    """
    ...
    start_parser.add_argument(
        "--background", "-d", action="store_true", help="Set this flag to run the worker in the background."
    )
    start_parser.add_argument(
        "--workers",
        "-w",
    ...
    start_parser.add_argument("--loglevel", "-l", type=str, default="INFO", help="Log level for the worker.")
    start_parser.add_argument(
        "--broker",
        "-b",
        type=str,
    ...
    start_parser.add_argument("additional_args", nargs="*", help="Additional arguments to pass to the worker.")
...
def get_validated_config(config_yaml: str | None = None) -> Any:
    ...
    if not config.success:
        msg = f"Errors while loading Taskiq executor config at {config_yaml}."
...
def get_config_module(config_yaml: str | None = None) -> str:
    ...
    with pathlib.Path(config_path).open("w", encoding="utf8") as fd:
        if validated_config.get("queue_url"):
            fd.write(f"QUEUE_URL = {validated_config['queue_url']!r}\n")
        if validated_config.get("region_name"):
            fd.write(f"REGION_NAME = {validated_config['region_name']!r}\n")
        if validated_config.get("endpoint_url"):
            fd.write(f"ENDPOINT_URL = {validated_config['endpoint_url']!r}\n")
        if validated_config.get("config_source"):
            fd.writelines(f"{key} = {value!r}\n" for key, value in validated_config["config_source"].items())

    return config_dir


def launch_background_worker(subprocess_args: list[str], env: dict[str, str] | None) -> subprocess.Popen[bytes]:
...
def worker_start_command(args: argparse.Namespace) -> int | None:  # noqa: C901
    ...
    Returns:
        Exit code if running in foreground, None if running in background
    """
    get_worker_name(args.name)
...
def dashboard_command(args: argparse.Namespace) -> None:
    ...
    """
    try:
        from taskiq_dashboard import TaskiqDashboard  # noqa: PLC0415
    except ImportError:
        return
    ...
    dashboard = TaskiqDashboard(
        api_token=args.api_token, broker=broker_instance, uvicorn_kwargs={"host": args.host, "port": args.port}
    )
    dashboard.run()


def worker_list_command(args: argparse.Namespace) -> None:
    ...
    queue_url = validated_config.get("queue_url", os.getenv("DAGSTER_TASKIQ_SQS_QUEUE_URL"))
    endpoint_url = validated_config.get("endpoint_url", os.getenv("DAGSTER_TASKIQ_SQS_ENDPOINT_URL"))
    region_name = validated_config.get("region_name", os.getenv("AWS_DEFAULT_REGION", "us-east-1"))
