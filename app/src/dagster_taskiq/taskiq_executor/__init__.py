...
class TaskIQExecutor(Executor):
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self.broker = LocalStackSqsBroker(settings)
    ...
    def _submit_step(self, plan_context: Any, step: Any) -> Iterator[DagsterEvent]:
        ...
        task = OpExecutionTask(
            ...
            step_key=step_key,
            op_name=step.op_def.name,
            op_config=step.op_config or {},
            inputs=self._resolve_inputs(step, plan_context),
            retry_count=0,
        )
        ...
        try:
            self.broker.kick_sync(task.to_json())
    ...
    def _poll_for_results(self, plan_context: Any, execution_plan: Any) -> Iterator[DagsterEvent]:
        ...
        timeout = self.settings.taskiq_visibility_timeout * 2  # Allow extra time
    ...
    def _is_already_completed(self, idempotency_key: str) -> bool:
        """Check if a task is already completed."""
        return self._idempotency_storage.is_completed(idempotency_key)
...
def taskiq_executor(init_context: InitExecutorContext) -> TaskIQExecutor:
    ...
    settings = Settings()
    return TaskIQExecutor(settings=settings)
